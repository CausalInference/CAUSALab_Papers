...
Purpose of Program: Run parametric g-formula for the the effect of a recommendation-based 
                    ...
                    Including people with missing or implausible baseline data and imputing these values
                    Also imputing missing values during follow-up when an item was missed but a questionnaire was returned
...
data bytimes;
    set gcomp.bytimes_prostate_impute;
...
* Impute missing baseline data using data available at baseline;
data bytimes_0;
    set bytimes;
...
* Impute using regression-based imputation;
proc mi data = bytimes_0 nimpute = 1 out = bytimes_0;  
    class stage
    gleason
    psacat
    txb
...
* To impute data using stochastic regression, the FCS statement is used. 
Within the FCS statement, you can impute using a discriminant function, 
logistic regression, regression, or predictive mean matching. 
Including the FCS statement will automatically use regression for continuous variables 
and discriminant functions for categorical variables.
For each imputed variable, all other variables in the VAR statement are used as covariates. 
An error term is added to each missing observation based on the observed variability of the data. 
;
...
proc freq data =  gcomp.bytimes_prostate_impute;
