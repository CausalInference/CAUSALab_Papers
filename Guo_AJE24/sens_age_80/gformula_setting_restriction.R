...
# This R program is to set the parametric g-formula settings for the sensitivity analysis
# that included individuals aged <80 years at baseline 
# and excused them from the physical activity intervention upon age 80 
# Code 1142 to 1164 contained an xcond_int() function that was different from the one
...
# here we require that the age to be intervened should be less than 80, otherwise, 
# we excuse them

xcond_int <- function(newdf, pool, intvar, intvals, time_name, t) {
  if (nrow(newdf[newdf[[intvar]] < intvals[[1]] & newdf[["xcond2"]] == 0 &
           ((newdf[["period"]])*2 + newdf[["age_base"]] < 80) ]) != 0) {
    classtmp <- class(newdf[[intvar]])
    myclass <- paste("as.", classtmp, sep = "")
    newdf[newdf[[intvar]] < intvals[[1]] & newdf[["xcond2"]] == 0 &
            ((newdf[["period"]])*2 + newdf[["age_base"]] < 80), 
          `:=`((intvar),  get(myclass)(intvals[[1]]))]
  }
  if (nrow(newdf[newdf[[intvar]] > intvals[[2]] & newdf[["xcond2"]] == 0 &
           ((newdf[["period"]])*2 + newdf[["age_base"]]  < 80) ]) != 0) {
    classtmp <- class(newdf[[intvar]])
    myclass <- paste("as.", classtmp, sep = "")
    newdf[newdf[[intvar]] > intvals[[2]] & newdf[["xcond2"]] == 0 
          &((newdf[["period"]])*2 + newdf[["age_base"]]  < 80) , 
          `:=`((intvar), get(myclass)(intvals[[2]]))]
  }
} 


# here we made a change to the default threshold function, just adding the requirement for
# age < 80

threshold <- function (newdf, pool, intvar, intvals, time_name, t) {
  if (nrow(newdf[newdf[[intvar]] < intvals[[1]] &
           ((newdf[["period"]])*2 + newdf[["age_base"]]  < 80)]) != 0) {
    classtmp <- class(newdf[[intvar]])
    myclass <- paste("as.", classtmp, sep = "")
    newdf[newdf[[intvar]] < intvals[[1]] &
            ((newdf[["period"]])*2 + newdf[["age_base"]]  < 80) , 
          `:=`((intvar), get(myclass)(intvals[[1]]))]
  }
  if (nrow(newdf[newdf[[intvar]] > intvals[[2]] &
           ((newdf[["period"]])*2 + newdf[["age_base"]] < 80)] ) != 0) {
    classtmp <- class(newdf[[intvar]])
    myclass <- paste("as.", classtmp, sep = "")
    newdf[newdf[[intvar]] > intvals[[2]] &
            ((newdf[["period"]])*2 + newdf[["age_base"]]  < 80) , 
          `:=`((intvar),  get(myclass)(intvals[[2]]))]
  }
}