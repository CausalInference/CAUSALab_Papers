...
# which expands the set of health conditions to excuse people from following the physical 
# activity intervention
# Code 1043 to 1059 contained an xcond_int() function that was different from the one
...
basecovs <- c("age_base", "fhxpca90", "smkhx", "hlthy_base", "fhxmi",
              ...
              "base_hbp", "base_chl", "pre_bmi", "pre_hlthy"
)
...
covparams <- list(
  covmodels = c(
    period_coarse ~ 1, 
    # first we model 4 histories diabetes, hbp, chl, and xcond3 based on values one lag before
...
censor_model <- censor ~ 
  ...
  as.numeric(pre_bmi>= 18.5 & pre_bmi < 25) + as.numeric(pre_bmi >= 25 & pre_bmi <30) + 
  as.numeric(pre_bmi>=30) +
  as.numeric(base_chl) + as.numeric(base_hbp) + 
  as.factor(period)
...
###############################################################
# that's the only difference from the primary analysis
# In the primary analysis we define xcond_int by using xcond2 while we define it by 
# xcond3 here.
xcond_int <- function(newdf, pool, intvar, intvals, time_name, t) {
  if (nrow(newdf[newdf[[intvar]] < intvals[[1]] & newdf[["xcond3"]] == 0]) != 0) {
    classtmp <- class(newdf[[intvar]])
    myclass <- paste("as.", classtmp, sep = "")
    newdf[newdf[[intvar]] < intvals[[1]] & newdf[["xcond3"]] == 0, `:=`((intvar), 
                                                                        get(myclass)(intvals[[1]]))]
  }
  if (nrow(newdf[newdf[[intvar]] > intvals[[2]] & newdf[["xcond3"]] == 0]) != 0) {
    classtmp <- class(newdf[[intvar]])
    myclass <- paste("as.", classtmp, sep = "")
    newdf[newdf[[intvar]] > intvals[[2]] & newdf[["xcond3"]] == 0, `:=`((intvar), 
